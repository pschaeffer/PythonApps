  ...
  for row in range(glbRowMin, glbRowMax+1):
    ...
    for value1 in range(glbValueMin, glbValueMax+1):
      ...
      for value2 in range(value1+1, glbValueMax+1):
        ...
        removeCount = removeAllowsExcludingListPair(tempCells, allowList, colIndex1, colIndex2) 
  ...
  for col in range(glbColMin, glbColMax+1):
    ...
    for value1 in range(glbValueMin, glbValueMax+1):
      ...
      for value2 in range(value1+1, glbValueMax+1):
        ...
        removeCount = removeAllowsExcludingListPair(tempCells, allowList, rowIndex1, rowIndex2) 
        totalRemoveAllowsCount += removeCount
        # Remove all other allows from the list pair
        removeCount = removeOtherAllowedValuesFromCells(cells1, allowList)
        totalRemoveAllowsCount += removeCount
  ...
  for box in range(glbBoxMin, glbBoxMax+1):
    ...
    for value1 in range(glbValueMin, glbValueMax+1):
      ...
      for value2 in range(value1+1, glbValueMax+1):
        ...
        if checkListsMatch(cells1, cells2) == False:
          continue
        # At this point we have a hidden (or possibly naked) pair. 
        # We can now adjust the allow and block lists for every 
        # cell in the unit
        pairCount += 1
        ...
        removeCount = removeAllowsExcludingListPair(tempCells, allowList, index1, index2) 
        totalRemoveAllowsCount += removeCount
        # Store the list of possible values in each cell
        for cell in cells1:
        ...
        if rowIndex1 == rowIndex2:
          ...
          # Remove allows from the rest of the row
          removeCount = removeAllowsExcludingListPair(tempCellsRow, allowList, colIndex1, colIndex2) 
          totalRemoveAllowsCount += removeCount
        ...
        if colIndex1 == colIndex2:
          ...
          # Remove allows from the rest of the column
          removeCount = removeAllowsExcludingListPair(tempCellsCol, allowList, rowIndex1, rowIndex2) 
          totalRemoveAllowsCount += removeCount
          ...
          for index4 in range(0, allowedListMergeLen):
            ...
            for index5 in range(index4+1, allowedListMergeLen):
              ...
              for index6 in range(index5+1, allowedListMergeLen):
                ...
                removeCount = removeAllowsExcludingListTriple(tempCells, tripleList, index1-1, index2-1, index3-1)
                totalRemoveAllowsCount += removeCount
            ...
            for index5 in range(index4+1, allowedListMergeLen):
              ...
              for index6 in range(index5+1, allowedListMergeLen):
                ...
                if rowIndex1 == rowIndex2 and \
                  ...
                  removeCount = removeAllowsExcludingListTriple(tempCellsRow, tripleList, colIndex1, colIndex2, colIndex3) 
                  totalRemoveAllowsCount += removeCount
                ...
                if colIndex1 == colIndex2 and \
                  ...
                  removeCount = removeAllowsExcludingListTriple(tempCellsCol, tripleList, rowIndex1, rowIndex2, rowIndex3)  
                  totalRemoveAllowsCount += removeCount
  ...
  for row in range(glbRowMin, glbRowMax+1):
    ...
    while index1 < glbColMax:
      ...
      while index2 < glbColMax:
        ...
        # Remove allows from the rest of the row
        removeCount = removeAllowsExcludingListPair(tempCells, allowList, index1-1, index2-1)  
        totalRemoveAllowsCount += removeCount
        # Check if the two cells are in the same box
    ...
    while index1 < glbColMax:
      ...
      while index2 < glbColMax:
        ...
        while index3 < glbColMax:
          ...
          tripleCount += 1
          # Remove all of the allowed values from the rest of the row
          removeCount = removeAllowsExcludingListTriple(tempCells, allowedListMerge, index1-1, index2-1, index3-1)
          totalRemoveAllowsCount += removeCount
          # Check if the three cells are in the same box
          if tempCell1.boxPosition == tempCell2.boxPosition and \
             tempCell2.boxPosition == tempCell3.boxPosition:
      ...
      while index2 < glbBoxMax:
        ...
        while index3 < glbBoxMax:
          ...
          if rowIndex1 == rowIndex2 and \
             rowIndex2 == rowIndex3:
            tempCellsRow = copyBoard.getRow(rowIndex1+1)
            removeCount = removeAllowsExcludingListTriple(tempCellsRow, allowedListMerge, colIndex1, colIndex2, colIndex3) 
            totalRemoveAllowsCount += removeCount
          # We can now check if the column positions match. If this
          # is true, then the rest of the column can be adjusted to 
          # remove the allowed values.
          if colIndex1 == colIndex2 and \
             colIndex2 == colIndex3:
            tempCellsCol = copyBoard.getCol(colIndex1+1)
            removeCount = removeAllowsExcludingListTriple(tempCellsCol, allowedListMerge, rowIndex1, rowIndex2, rowIndex3)  
            totalRemoveAllowsCount += removeCount
    ...
    for box in range(glbBoxMin, glbBoxMax+1):
      ...
      if count == 3:
        ...
        if rowIndex1 == rowIndex2 and \
          ...
          pairTripleCount += 1
          tempCellsRow = copyBoard.getRow(rowIndex1+1)
          removeCount = removeAllowsExcludingListTriple(tempCellsRow, allowList, colIndex1, colIndex2, colIndex3) 
          totalRemoveAllowsCount += removeCount
        # We can now check if the column positions match. If this
        # is true, then the rest of the column can be adjusted to 
        # remove the allowed values.
        if colIndex1 == colIndex2 and \
          ...
          pairTripleCount += 1
          tempCellsCol = copyBoard.getCol(colIndex1+1)
          removeCount = removeAllowsExcludingListTriple(tempCellsCol, allowList, rowIndex1, rowIndex2, rowIndex3) 
          totalRemoveAllowsCount += removeCount
...
# been actually removed (of course, the count may be
# zero).
def removeAllowsExcludingListPair(curCells, allowList, index1, index2):
  removedCount = 0
  # Get the length of the list of cells passed to this routine
  curCellsLen = len(curCells)
  for allowValue in allowList:
...
# been actually removed (of course, the count may be
# zero).
def removeAllowsExcludingListTriple(curCells, allowList, index1, index2, index3):
  removedCount = 0
  # Get the length of the list of cells passed to this routine
  curCellsLen = len(curCells)
  for allowValue in allowList:
